...
`endif
  ...
  // Below is the easily understandable version (commented out) and replaced
  // with an optimized but hard to read version (see further below).
  /*
  // For deterministic routing, we always choose the same decision bit.
  wire decision_bit = 1'b0;
  ...
  */
  ...
  assign req[W] = ~x_eq & x_lt;
  assign req[E] = ~x_eq & x_gt;
