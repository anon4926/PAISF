module bsg_mesh_router_buffered (
	...
	wire [(dirs_lp * 7) - 1:0] internal_fifo_count;
	genvar i;
	generate
	...
	generate
		for (i = 0; i < dirs_lp; i = i + 1) begin : rof
			if (stub_p[i]) begin : fi
				...
				assign internal_fifo_count[i * 7+:7] = fi.fifo.count_o;
				always @(negedge clk_i)
					;
			end
			else begin : fi
	...
	bsg_mesh_router #(
		...
		.credit_count_i(internal_fifo_count),
		.my_x_i(my_x_i),
		.my_y_i(my_y_i),
...
module bsg_mesh_router (
	...
	input [(in_dirs_lp * 7) - 1:0] credit_count_i;
	input [x_cord_width_p - 1:0] my_x_i;
	input [y_cord_width_p - 1:0] my_y_i;
	input [5:0] router_id_val_i;
	...
	generate
		for (i = 0; i < in_dirs_lp; i = i + 1) begin : dor
			...
			bsg_mesh_router_decoder_dor #(
				...
				.credit_counts_i(credit_count_i),
				.req_o(temp_req)
			);
...
module bsg_mesh_router_decoder_dor (
	...
	input [(dirs_lp * 7) - 1:0] credit_counts_i;
	...
	wire ns_more_credits_than_we = credit_counts_i[(y_lt ? sv2v_cast_3((3'd0 + 1) + 2) : sv2v_cast_3((3'd0 + 1) + 3)) * 7+:7] < credit_counts_i[(x_lt ? sv2v_cast_3(3'd0 + 1) : sv2v_cast_3((3'd0 + 1) + 1)) * 7+:7];
	reg [2:0] last_entropy_val;
	reg stored_decision_bit;
	always @(posedge clk_i)
		if (reset_i) begin
			last_entropy_val <= 3'b000;
			stored_decision_bit <= 1'b0;
		end
		else if (entropy_val != last_entropy_val) begin
			last_entropy_val <= entropy_val;
			stored_decision_bit <= ns_more_credits_than_we;
		end
	wire adaptive_res = (entropy_val == last_entropy_val ? stored_decision_bit : ns_more_credits_than_we);
	...
	assign req[sv2v_cast_3(3'd0 + 1)] = (x_eq ? 1'b0 : (y_eq ? x_lt : (x_lt ? 1'b1 : (decision_bit ? 1'b0 : x_lt))));
