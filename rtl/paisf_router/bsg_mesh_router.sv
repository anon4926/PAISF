...
module bsg_mesh_router
  ...
  (
	...
	, input [in_dirs_lp-1:0][6:0] credit_count_i // New credit input
  ...
  logic [in_dirs_lp-1:0][2:0] entropy_vals;
  ...
  for (genvar i = 0; i < in_dirs_lp; i++) begin
	...
	// Extract 3-bit entropy starting after X and Y coordinates
    assign entropy_vals[i] = data_i[i][(x_cord_width_p + y_cord_width_p)+:3];
  end
  ...
  for (genvar i = 0; i < in_dirs_lp; i++) begin: dor
    ...
    bsg_mesh_router_decoder_dor #(
	  ...
	  // Pass the extracted entropy for this direction
      , .entropy_val(entropy_vals[i])

      // Pass the current router's ID (or specific ID if needed per port)
	  , .router_id_val(router_id_val_i)

	  // Pass the specific credit count for port 'i'
