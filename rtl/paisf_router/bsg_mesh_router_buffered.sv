...
module bsg_mesh_router_buffered
  ...
  #(parameter width_p         = 64               // Hard-coded value
    , parameter x_cord_width_p = 3               // Hard-coded value
    , parameter y_cord_width_p = 3               // Hard-coded value
    , parameter debug_p        = 0
    ...
    , parameter int fifo_els_p[dirs_lp-1:0] = '{8,8,8,8,8}
  )
  (
    input clk_i
    , input reset_i
   ...
   // Custom signal for number of credits (for 64-flit buffers)
   logic [dirs_lp-1:0][6:0] internal_fifo_count;
  ...
  for (i = 0; i < dirs_lp; i=i+1) begin: rof
    if (stub_p[i]) begin: fi
      ...
      assign link_o_cast[i].ready_and_rev = 1'b0;
  	  ...
  	  // Assign the count directly outward from inside the block
      assign internal_fifo_count[i] = fi.fifo.count_o;
...
`endif
   ...
   (.clk_i
	...
	// Connect the internal signal here:
