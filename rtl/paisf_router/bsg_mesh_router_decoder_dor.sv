...
module bsg_mesh_router_decoder_dor
  ...
  (
	...
	, input [dirs_lp-1:0][6:0] credit_counts_i  // ADDED: Credit count for this specific port
...
`endif
  ...
  localparam bit [7:0] HASH_CONST = 8'hB9;
  ...
  // Credit count is the number of credits used, not available.
  wire ns_more_credits_than_we = (credit_counts_i[y_lt ? N : S] < credit_counts_i[x_lt ? W : E]);

  // State registers
  logic [2:0] last_entropy_val;
  logic       stored_decision_bit;

  always_ff @(posedge clk_i) begin
    if (reset_i) begin
      last_entropy_val    <= 3'b0;
      stored_decision_bit <= 1'b0;
    end else begin
      // If entropy changes, update the state
      if (entropy_val != last_entropy_val) begin
  ...
  wire decision_bit = from_p[P] ? adaptive_res : decision_bit;
